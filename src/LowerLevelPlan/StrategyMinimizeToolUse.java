...
public class StrategyMinimizeToolUse implements DataIO {
	...
	public StrategyMinimizeToolUse(BlackBoard bb) {
	...
	public List<Integer> insertTask(List<Integer> route, int task) {
		...
		for (int i = 1; i < route.size(); i++) {
			if (route.get(i) != 0) {
				int curReqId = route.get(i);
				List<Integer> r = new ArrayList<>(route);
				if (checkAssociation(curReqId, task)) {
					r = insertTaskToPos(task, i + 1, r, 2);
					bestRoute = new ArrayList<>(r);
					curDistance = distanceCheckPerRoute(r);
					continue;
				} else if (checkAssociation(task, curReqId)) {
					r = insertTaskToPos(task, i, r, 3);
					bestRoute = new ArrayList<>(r);
					curDistance = distanceCheckPerRoute(r);
					continue;
				}
	...
	public boolean checkAssociation(int curReqId, int insertReqId) {
		Request curReq = requests.get(Math.abs(curReqId) - 1);
		if (curReqId < 0 && insertReqId > 0 && curReq.getAssociateSet().contains(insertReqId)) {
			return true;
